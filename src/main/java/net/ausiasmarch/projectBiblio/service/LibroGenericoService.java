...
public class LibroGenericoService {
    ...
    private String IsbnRandom() {
        return "978-" + oRandomService.getRandomInt(0, 999) + "-" + oRandomService.getRandomInt(0, 99999) + "-"
                + oRandomService.getRandomInt(0, 999) + "-" + oRandomService.getRandomInt(0, 9);
    }

    public Long randomCreate(Long cantidad) {

        arrayTitulos = new String[] { "El Quijote", "La Celestina", "El Lazarillo de Tormes", "La Regenta",
                "Fortunata y Jacinta", "La Colmena", "El Jarama", "El camino", "La familia de Pascual Duarte", "Nada",
                "La sombra del viento", "Los pilares de la tierra", "El nombre de la rosa", "Cien años de soledad",
                "Rayuela", "Pedro Páramo", "Don Juan Tenorio", "La casa de Bernarda Alba", "Bodas de sangre", "Yerma" };
        arrayAutores = new String[] { "Miguel de Cervantes", "Fernando de Rojas", "Anónimo", "Leopoldo Alas Clarín",
                "Benito Pérez Galdós", "Camilo José Cela", "Rafael Sánchez Ferlosio", "Miguel Delibes",
                "Camilo José Cela", "Carmen Laforet", "Carlos Ruiz Zafón", "Ken Follet", "Umberto Eco",
                "Gabriel García Márquez", "Julio Cortázar", "Juan Rulfo", "Tirso de Molina", "Federico García Lorca",
                "Federico García Lorca", "Federico García Lorca" };
        arrayEditorial = new String[] { "Alfaguara", "Anaya", "Santillana", "SM", "Vicens Vives", "Oxford", "Cambridge",
                "Pearson", "Macmillan", "Longman", "Richmond", "Express Publishing", "McGraw Hill", "Hachette",
                "Larousse", "Difusión", "Edelsa", "Edebe", "Burlington", "Cornelsen" };
        arrayDescripcion = new String[] { "Libro de aventuras", "Libro de amor", "Libro de misterio", "Libro de terror",
                "Libro de ciencia ficción", "Libro de fantasía", "Libro de intriga", "Libro de humor",
                "Libro de viajes", "Libro de historia", "Libro de biografía", "Libro de ensayo", "Libro de poesía",
                "Libro de teatro", "Libro de novela", "Libro de cuento", "Libro de relato", "Libro de cómic",
                "Libro de manga", "Libro de novela gráfica" };
        ...
        for (int i = 0; i < cantidad; i++) {
            ...
            oLibroGenericoEntity
                    .setEditorial(arrayEditorial[oRandomService.getRandomInt(0, arrayEditorial.length - 1)]);
            oLibroGenericoEntity.setIsbn("ISBN" + IsbnRandom());
            oLibroGenericoEntity
                    .setAnio(Long.valueOf(oRandomService.getRandomInt(1900, LocalDateTime.now().getYear())));
            oLibroGenericoEntity
                    .setDescripcion(arrayDescripcion[oRandomService.getRandomInt(0, arrayDescripcion.length - 1)]);
    ...
    public Page<LibroGenericoEntity> getPage(Pageable oPageable, Optional<String> filter) {
        if (oAuthService.isAdmin() || oAuthService.isContable() || oAuthService.isAuditor()) {
            if (filter.isPresent()) {
                return oLibroGenericoRepository
                        .findByTituloContaining(oPageable, filter.get());
            } else {
    ...
    public LibroGenericoEntity update(LibroGenericoEntity oLibroGenericoEntity) {
        if (oAuthService.isContableWithItsOwnData(oLibroGenericoEntity.getId()) || oAuthService.isAdmin()
            ...
            LibroGenericoEntity oLibroGenericoEntityFromDatabase = oLibroGenericoRepository
                    .findById(oLibroGenericoEntity.getId()).get();
